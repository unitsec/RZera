# 功能代码，继承UI代码，给UI控件添加功能; groupSA对应SA groupMA对应MA groupHA对应HA
from PyQt5 import QtWidgets
from PyQt5 import QtGui
from PyQt5.QtCore import Qt
from BL15_HPND.ui.ui_BL15_diffraction_small import Ui_Form
from matplotlib.backends.backend_qt5agg import FigureCanvasQTAgg as FigureCanvas
from matplotlib.backends.backend_qt5agg import NavigationToolbar2QT as NavigationToolbar
from matplotlib.figure import Figure
from BL15_HPND.workers.browse_file import browse_file
from BL15_HPND.workers.combobox_operation import browse_mode_check, change_rebin_mode
from BL15_HPND.workers.checkbox_operation import checkbox_operation
from utils.NonZeroIntValidator import NonZeroIntValidator
from BL15_HPND.workers.plot_item_operation import change_item
from BL15_HPND.workers.data_plot import data_plot
from BL15_HPND.workers.ui_mapping_small import save_diff_config, load_diff_config
from BL15_HPND.workers.data_save import save_diff_small
import BL15_HPND.workers.diffraction_thread_small
import json
...
class diffraction_small(QtWidgets.QWidget, Ui_Form):
    def __init__(self, parent=None):
        super(diffraction_small, self).__init__(parent)
        self.setupUi(self)
        ...
        if getattr(sys, 'frozen', False):
            ...
            bl15_config = os.path.join(temp_dir, 'BL15_small_base.json')
        else:
            # 如果是在源码中运行
            temp_dir = os.path.abspath(os.path.dirname(__file__))
            bl15_config = os.path.join(temp_dir, '..', 'CSNS_Alg', 'configure', 'BL15_small_base.json')

        with open(bl15_config, 'r', encoding='utf-8') as json_file:
            self.bl15_configure = json.load(json_file)
        ...
        self.sample_run_button.clicked.connect(
            lambda: (self.browse_run.select_nxsfiles(self.sample_run_text, self.diff_config['sam_fn']),
                     print(self.diff_config['sam_fn'])))
        ...
        self.num_dens.setText(str(self.bl15_configure['vanadium_correction']['density_num']))
        self.num_dens.setValidator(doubleValidator)
        self.van_radius.setEnabled(False)
        self.van_radius.setText(str(self.bl15_configure['vanadium_correction']['radius']))
        self.van_radius.setValidator(doubleValidator)
        self.beam_height.setEnabled(False)
        self.beam_height.setText(str(self.bl15_configure['vanadium_correction']['beam_height']))
        ...
        self.v_check.stateChanged.connect(
            lambda: (self.checkbox_operation.check_set(self.v_check, self.van_run_button, self.van_run_text,
                     ...
                     self.checkbox_operation.sync_checkboxes(self.v_check,
                                                             self.sam_van_check)))  # 根据 v_check 的勾选状态设置van_run_button、van_run_text、van_run_mode的可用状态
        try:
            self.van_run_mode.currentTextChanged.connect(
                lambda: (self.van_run_text.setText(''),
                         self.diff_config.update({'v_fn': []})))  # van_run_mode的选择变化会清空van_run_filePath和van_run_text
        ...
        self.vbkg_check.stateChanged.connect(
            lambda: (
            self.checkbox_operation.check_set(self.vbkg_check, pushbutton=self.vanbkg_button, lineedit=self.vanbkg_text,
                                              combobox=self.vanbkg_mode),
        ...
        #################################### 配置 d_rebin  #######################################################
        self.groupSA_start.setValidator(doubleValidator)  # 将doubleValidator设置到 QLineEdit 控件上(只能输入浮点数)
        self.groupSA_end.setValidator(doubleValidator)
        self.groupMA_start.setValidator(doubleValidator)
        self.groupMA_end.setValidator(doubleValidator)
        self.groupHA_start.setValidator(doubleValidator)
        self.groupHA_end.setValidator(doubleValidator)

        self.nozerointvalidator = NonZeroIntValidator(1, 2147483647)  # 实例化自定义的NonZeroIntValidator
        self.groupSA_number.setValidator(self.nozerointvalidator)  # 将NonZeroIntValidator设置到 QLineEdit 控件上(只能输入正整数)
        self.groupMA_number.setValidator(self.nozerointvalidator)
        self.groupHA_number.setValidator(self.nozerointvalidator)

        self.checkbox_operation = checkbox_operation()
        self.checkbox_operation.check_set(self.groupSA_check, lineedit=self.groupSA_start)
        self.checkbox_operation.check_set(self.groupSA_check, lineedit=self.groupSA_end)
        self.checkbox_operation.check_set(self.groupSA_check, lineedit=self.groupSA_number)
        self.checkbox_operation.check_set(self.groupMA_check, lineedit=self.groupMA_start)
        self.checkbox_operation.check_set(self.groupMA_check, lineedit=self.groupMA_end)
        self.checkbox_operation.check_set(self.groupMA_check, lineedit=self.groupMA_number)
        self.checkbox_operation.check_set(self.groupHA_check, lineedit=self.groupHA_start)
        self.checkbox_operation.check_set(self.groupHA_check, lineedit=self.groupHA_end)
        self.checkbox_operation.check_set(self.groupHA_check, lineedit=self.groupHA_number)

        self.groupSA_check.stateChanged.connect(
            lambda: (self.checkbox_operation.set_bank(self.groupSA_check, self.groupSA_start, config=self.bl15_configure),
                     self.checkbox_operation.set_bank(self.groupSA_check, self.groupSA_end, config=self.bl15_configure),
                     self.checkbox_operation.set_bank(self.groupSA_check, self.groupSA_number,
                                                      config=self.bl15_configure)))
        self.groupMA_check.stateChanged.connect(
            lambda: (self.checkbox_operation.set_bank(self.groupMA_check, self.groupMA_start, config=self.bl15_configure),
                     self.checkbox_operation.set_bank(self.groupMA_check, self.groupMA_end, config=self.bl15_configure),
                     self.checkbox_operation.set_bank(self.groupMA_check, self.groupMA_number,
                                                      config=self.bl15_configure)))
        self.groupHA_check.stateChanged.connect(
            lambda: (self.checkbox_operation.set_bank(self.groupHA_check, self.groupHA_start, config=self.bl15_configure),
                     self.checkbox_operation.set_bank(self.groupHA_check, self.groupHA_end, config=self.bl15_configure),
                     self.checkbox_operation.set_bank(self.groupHA_check, self.groupHA_number,
                                                      config=self.bl15_configure)))

        ######################################### 配置 reduction button  ################################################
        self.reduction_thread = BL15_HPND.workers.diffraction_thread_small.start_reduction_thread()  # 实例化start_reduction_thread
        ...
        ######################################### 配置 save_config  #######################################################
        self.save_config.clicked.connect(lambda: save_diff_config(self, self.diff_config, self.bl15_configure))
        ...
        self.plot_button.clicked.connect(lambda: (
            self.data_plot.plot_data(self.sam_list, self.sam_list_dict, self.canvas, self.ax, 'd (Å)',
                                     'Intensity (a.u.)'),
            self.data_plot.plot_data(self.other_list, self.other_list_dict, self.canvas, self.ax, 'd (Å)',
                                     'Intensity (a.u.)', False)
        )
        ...
        self.save_button.clicked.connect(lambda:
                                         save_diff_small(self, self.diff_config,
