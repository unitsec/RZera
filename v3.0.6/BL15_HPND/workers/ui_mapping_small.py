...
def diff_mapping(window, diff_config, base_config):
    ...
    base_config['d_rebin'] = {
        "groupSA": [float(diff_config['groupSA_start']) if diff_config['groupSA_start'].strip() else None,
                  float(diff_config['groupSA_end']) if diff_config['groupSA_end'].strip() else None,
                  int(diff_config['groupSA_number']) if diff_config['groupSA_number'].strip() else None],
        ...
        "groupHA": [float(diff_config['groupHA_start']) if diff_config['groupHA_start'].strip() else None,
                  float(diff_config['groupHA_end']) if diff_config['groupHA_end'].strip() else None,
                  int(diff_config['groupHA_number']) if diff_config['groupHA_number'].strip() else None]}
        

    diff_config['group_list'] = []
    for group in ['groupSA', 'groupMA', 'groupHA']:
...
def save_diff_config(window, diff_config, bl15_config):
    try:
        diff_config, _ = diff_mapping(window, diff_config, bl15_config)
