import traceback
import numpy as np
from rongzai.algSvc.instrument import run_module_offset
...
class CSNS_Offset():
    ...
    def calculate_offset(self):
        ...
        for module in self.moduleList:
            print("start: ", module, self.conf["sample_fns"])
            dataset = load_histogram_data(self.conf["sample_fns"], f'{self.conf["param_path"]}/{module}.txt',
                                          module, self.conf["first_flight_distance"],
                                          x_offset=float(self.conf["T0_offset"]))
            plot_info_module = run_module_offset(dataset, self.conf, self.is_ui)
            plot_info_all.append(plot_info_module)
    ...
    #     for module in self.moduleList:
    #         # print("start: ", module,self.conf["sample_fn"])
    #         dataset = load_neutron_data(self.conf["sample_fns"], f'{self.conf["param_path"]}/{module}.txt',
    #                                     module, self.conf["first_flight_distance"])
    #
    ...
    #         run_module_offset(dataset,self.conf,self.ui)
            # if self.pixelInfo[module]["stepbyrow"] == "x":
            #     pixels = self.pixelInfo[module]["xpixels"]
            # else:
            #     pixels = self.pixelInfo[module]["ypixels"]
    ...
    #     data = load_neutron_data(self.conf["sample_fns"], f'{self.conf["param_path"]}/{module}.txt',
    #                                  module, self.conf["first_flight_distance"])
