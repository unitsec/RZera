from utils.ui import load_beamline_config, load_and_focus, cut_peaks, smooth, save_focused_data, get_event_data
...
class TimeFocusing(RunWidget):
    def __init__(self, parent=None):
        ...
        self.data_list = [{
                            ...
                            "correction_info":carpenterCorrection,
                            "record":{
                                    "start_time"(optional):"{start_time}"(str),
                                    "end_time"(optional):"{end_time}"(str),
                                    "mask"(optional):"{mask_folder}"(str),
                                    "crop"(optional):"{wavemin}_{wavemax}"(str),
                                    “carpenterCorrection”(optional):{
                                                                "sample_name":(str),
                                                                "mass"(str),
                                                                "cell_radius":cell_radius(float),
                                                                "volume":{
                                                                        "type":"cylinder"(str),
                                                                ...
                                                                "scale":1(float),
                                                                "density_num": the number density of sample(float) 
                                                                }(dict),
                                    "normalization"(optional):"proton_charge" or "monitor_counts"(str),
                                    "d_rebin"(optional):[start(float),step(float),num(int)](list),
                                    “scaleBkg”(optional):same with carpenterCorrection (might without "density_num")
                                    "offset"(optional):"{offset_folder}"(str),
                                    "subtraction_self":"{sambkgdata["name"]}", #只有被扣背底的数据会带这个key
                                    "subtraction_v":"{vbkgdata["name"]}", #只有样品数据且其刻度V数据被扣过背底的，才会带这个key
                                    "division":"{vdata["name"]}", #只有被除V的数据会带这个key
                                    ...,
                                    "":str
                                      },
                            },
        ...
        self.get_event_data = self.add_module(get_event_data, "Get Event Data")
