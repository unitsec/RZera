...
class checkbox_operation:
    ...
    def set_bank(self, checkbox, lineedit, use_pushbutton=False, use_combobox=False, pushbutton=None,  combobox=None, config=None):
        ...
        if lineedit.isEnabled() and config is not None:
            ...
            if object_name.endswith('start'):
                lineedit.setText(str(config['d_rebin'][object_name[:6]][0]))
            elif object_name.endswith('end'):
                lineedit.setText(str(config['d_rebin'][object_name[:6]][1]))
            elif object_name.endswith('number'):
                lineedit.setText(str(config['d_rebin'][object_name[:6]][2]))
        else:
            lineedit.setText('')
