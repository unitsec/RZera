...
class save_all_data(CollapsibleWidget):
    def __init__(self, parent):
        super(save_all_data, self).__init__("Save All Data", "utils/ui/save_all_data.ui", parent)
        self.parent = parent
            ...
            if self.toggle_button.isChecked():
                ...
                for data in self.parent.data_list:
                    if 'detector_focused' in data.keys():
                        ...
                        print(data['name'])
                        datatype = self.get_datatype_from_name(data['name'])
                        if datatype:
                        ...
                        check_dir(path)
                        # 保存 I-d 数据
                        fn = os.path.join(path, f"{data['name'].replace(':', '')}.txt")
                        ...
                        fn = os.path.join(path, f"{runno}_{data['detector']}")
