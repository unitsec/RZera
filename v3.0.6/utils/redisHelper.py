...
class RedisHelper(IOBase):
    ...
    def readNumber(self, path):
        byte_string = self.read(path)
    ...
    def writeNumber(self, path, value):
        byte_string = str(value).encode('utf-8')
        self.write(path, byte_string)
...
def getredisPath(confFile):
    if not os.path.exists(confFile):
        print("ERROR:No Redis Path file!")
    else:
        paths =json.loads(open(confFile, "r").read())
        return paths