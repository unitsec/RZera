...
class redisThread(QThread):
    bank_reduction = pyqtSignal(np.ndarray,np.ndarray,str)
    data_ready = pyqtSignal(dict)
    bank_1d = pyqtSignal(np.ndarray,np.ndarray,str)
    bank_1d_from_unit = pyqtSignal(np.ndarray, np.ndarray, str)
    proton_charge = pyqtSignal(np.ndarray, np.ndarray, str)
    request_snapshot = pyqtSignal(str, str, str)  # filename, tab_name, remote
    # bank_2d = pyqtSignal(np.ndarray, str)
    bank_2d = pyqtSignal(dict)
    image = pyqtSignal(np.ndarray)
    sqw = pyqtSignal(list, list, list)
    module_1d = pyqtSignal(np.ndarray,np.ndarray)
    monitor_1d = pyqtSignal(np.ndarray,np.ndarray)
    monitor_1d_BL05 = pyqtSignal(np.ndarray,np.ndarray)
    monitor_2d = pyqtSignal(np.ndarray)
    ei = pyqtSignal(np.ndarray)
    reduction_ready = pyqtSignal(dict)
    dataMonitorLog = pyqtSignal(np.ndarray)  # 用于发送日志信息到数据监控界面
    ...
    def get_data_from_imacs(self):
        logList = []
        for key, path in self.allPath.items():
            if "counts_hist" in key:
                data = self.rds.readNumpyArray(path)
                if data is None or data.size == 0:
                    logList.append({"path": path, "status": "no_data"})
                elif np.count_nonzero(data) == 0:
                    logList.append({"path": path, "status": "all_zero"})
            elif "userID" in key or "proposalID" in key:
                value = self.rds.readStr(path)
                ...
                if value is None or value == "":
                    ...
                    logList.append({"path": path, "status": "invalid_runno"})
            elif "proposalID" in key:
                value = self.rds.readStr(path)
                if value is None or value == "":
                    logList.append({"path": path, "status": "no_data"})
                elif not re.match(r'^[A-Za-z0-9]+$', value):
                    logList.append({"path": path, "status": "invalid_proposalID"})
            elif "proton_charge_hist" in key or "detector_counts_hist" in key:
                data = self.rds.readNumpyArray(path)
                if data is None or data.size == 0:
                    logList.append({"path": path, "status": "no_data"})
        return logList

    def get_bank_reduction(self,bank):
        try:
            # 重写key
            # unit = self.parent.get_bank_unit()
            unit = 'd'
    ...
    def get_bank_1d_from_unit(self,bank, unit):
        try:
            key = bank+"_"+unit+"_raw"
            path1 = self.allPath[key]
            key = bank+"_"+unit+"_counts_raw"
    ...
    def set_wechat_new(self, BL_name, tab_name, wechat_map, detectorList=None):
        runno = self.rds.readStr(self.allPath["runno"])
        runno = "RUN" + str(runno).zfill(7)

        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        history_folder = os.path.join(project_root, BL_name, "online", "history")
        file_Path = os.path.join(history_folder, runno)
        self.check_dir(file_Path)
        # 这些都可以留在工作线程（仅 Redis + 文件 I/O）
        self.writeWechatInfo(file_Path)
        if detectorList:
            for detector in detectorList:
                self.writeWechatData(file_Path, detector, wechat_map)
        filename = os.path.join(file_Path, wechat_map[tab_name] + ".jpg")
        remote = f"tangm@cportal.csns.ihep.ac.cn:/home/tangm/instrument/{BL_name[:4]}"
        # 不再在工作线程里截图与上传，只发请求给主线程
        self.request_snapshot.emit(filename, tab_name, remote)
    ...
    def set_wechat(self, BL_name, tab_name, wechat_map, detectorList):
        ...
        time.sleep(1)
        _cmd = 'scp -r ' + file_Path + ' tangm@cportal.csns.ihep.ac.cn:/home/tangm/instrument/' + BL_name[:4]
    ...
    def set_user(self,infoList):
        try:
            ...
            for name in infoList:
                ...
                # print(f"{name}:",self.allPath[name], data[name])
    ...
    def set_dataMonitorLog(self):
        try:
            logList = self.get_data_from_imacs()
            logArray = np.array(logList, dtype=object)  # 转换为NumPy数组
            self.dataMonitorLog.emit(logArray)  # 发送信号
        except Exception as e:
            traceback.print_exc()  # 打印异常的堆栈跟踪
    ...
    def set_bank_reduction(self,bankList):
        try:
            # merged_bank ={'bank1to6': [], 'bank7_14': [], 'bank8_13': [], 'bank9_12': [], 'bank10_11': []}
            for bank in bankList:
                x,y = self.get_bank_reduction(bank)
                # print(f'{bank} data:',np.shape(x),np.shape(y))
                x,y = self.crop_zero_data(x,y)
                # print(x, y)
                self.bank_reduction.emit(x, y, bank)
                    # self.parent.set_bank_1d(x, y, bank)
        except Exception as e:
            traceback.print_exc()  # 打印异常的堆栈跟踪

    def set_bank_1d_from_unit(self,bankList, unit):
        try:
            for bank in bankList:
                x,y = self.get_bank_1d_from_unit(bank,unit)
                # print(f'{bank} data:',np.shape(x),np.shape(y))
                x,y = self.crop_zero_data(x,y)
                # x = np.array([])
                # y = np.array([])
                self.bank_1d_from_unit.emit(x, y, bank)
                    # self.parent.set_bank_1d(x, y, bank)
        except Exception as e:
            traceback.print_exc()  # 打印异常的堆栈跟踪
...
################################################# set BL05_hd ########################################
    def get_image(self):
        path = self.allPath["image_counts"]
        image = self.rds.readNumpyArray(path)
        print(f"get from redis path {path}")
    ...
    def set_monitor_2d(self):
        try:
            z = self.get_monitor_2d()
            self.monitor_2d.emit(z)
        except Exception as e:
            logging.error("An error occurred", exc_info=True)

    def set_sqw(self, history=False):
        try:
            path = self.allPath["ei"]
            ei = self.rds.readNumpyArray(path)
            # print(ei)
            sqws = []
            qs = []
            energys = []
            ...
            for i in range(len(ei)):
                if history:
                    sqw = self.rds.readNumpyArray(f"/hd/rongzai/history/ei{i}/sqw")
                    q = self.rds.readNumpyArray(f"/hd/rongzai/history/ei{i}/q")
                    energy = self.rds.readNumpyArray(f"/hd/rongzai/history/ei{i}/energy")
                else:
                    sqw = self.rds.readNumpyArray(f"/hd/rongzai/ei{i}/sqw")
                    q = self.rds.readNumpyArray(f"/hd/rongzai/ei{i}/q")
                    energy = self.rds.readNumpyArray(f"/hd/rongzai/ei{i}/energy")
                # print(f"sqw:{sqw}")
                # print(f"q:{q}")
    ...
    def set_ei(self):
        try:
            ...
            ei = self.rds.readNumpyArray(path)
            # print(ei)
            # test = self.rds.readStr(f"/hd/imacs/ei")
            # print(test)
            # print(ei)
            self.ei.emit(ei)
        except Exception as e:
    ...
    def set_monitor_1d_BL05(self):
        try:
            x, y = self.get_monitor_1d()
            self.monitor_1d_BL05.emit(x, y)
        except Exception as e:
            logging.error("An error occurred", exc_info=True)
########################################################

###################### set BL16 reduction #################
    def set_reduction(self,infoList):
        try:
            data = {}
            for name in infoList:
                data[name]=self.rds.readStr(self.allPath[name])
            ...
            #data["test"] = self.rds.readStr("/emd/control/run_control")
            #print(data)
            self.reduction_ready.emit(data)
            # self.parent.set_user(data)
        except Exception as e:
            traceback.print_exc()  # 打印异常的堆栈跟踪